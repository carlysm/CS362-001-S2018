...
//Change for loop counter to start at 1 instead of 0.
//Any recurrences on the first day of the week will not be registered by the program, so the next occurrence might actually be skipped.
//FIX. FOUND BY TESTS
                for (int k = 0; k < 7; k++) {
                    nextDay.add(nextDay.DAY_OF_MONTH, 1);
